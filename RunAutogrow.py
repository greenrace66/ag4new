...
PARSER.add_argument(
    "--docking_energy_range",
    type=float,
    default=None,
    help="VINA score cutoff. Each output pose score has to fall in the range \
    top_score < x < top_score + docking_energy_range. Unless specified Autogrow \
    uses the docking softwares default settings. For AutoDock Vina 1.1.2 is 3",
)
####
PARSER.add_argument(
...
PARSER.add_argument(
    ...
    type=str,
    default="",
    help="reference compound pose in PDBQT format. NECESSARY for scoring functions \
    based on interaction fingeprint from the Open Drug Discovery Toolkit Python library \
    (ODDT), such as ODDT_IFP.",
)   
####
PARSER.add_argument(
