...
class VinaDocking(ParentDocking):
    ...
    def dock_ligand(self, lig_pdbqt_filename):
        ...
        if vars["docking_energy_range"] is not None and vars["docking_energy_range"] != "None":
            if (
                    type(vars["docking_energy_range"]) == int
                    or type(vars["docking_energy_range"]) == float
            ):
                torun = torun + " --energy_range " + str(int(vars["docking_energy_range"]))        
