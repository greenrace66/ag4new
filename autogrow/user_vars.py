...
def check_for_required_inputs(input_params):
    ...
    #### modified by Matteo Pavan
    if "docking_energy_range" in list(input_params.keys()):
        if input_params["docking_energy_range"] == "None":
            input_params["docking_energy_range"] = None
        if input_params["docking_energy_range"] is not None:
            try:
                input_params["docking_energy_range"] = int(
                    input_params["docking_energy_range"]
                )
            except:
                pass

            if (
                    type(input_params["docking_energy_range"]) != int
                    and type(input_params["docking_energy_range"]) != float
            ):
                raise Exception(
                    "docking_energy_range needs to be an interger. \
                    If you do not know what to use, leave this blank and the \
                    default for the docking software will be used."
                )    
    ####
    ...
    #### modified by Matteo Pavan
    if "reference_ligand" in list(
            input_params.keys()
    ):
        input_params["reference_ligand"] = os.path.abspath(
            input_params["reference_ligand"]
        )
            # Check reference_ligand exists
        if os.path.isfile(input_params["reference_ligand"]) is False:
            raise NotImplementedError(
                "reference_ligand can not be found. \
                File must be a .pdbqt file."
            )
        if ".pdbqt" not in input_params["reference_ligand"]:
            raise NotImplementedError(
                "reference_ligand must be a \
                .pdbqt file."
            )            
    ####
...
def define_defaults():
    ...
    #### modified by Matteo Pavan
    vars["docking_energy_range"] = None
    ####
